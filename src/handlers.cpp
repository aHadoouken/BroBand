...
Handlers::add_user_request(User &user) {
    ...
    if (!user.profile_avatar.isEmpty()) {
        ...
        if (file.good()) {
            ...
            beast::detail::base64::encode((void *) avatar_base64.data(),
                                          avatar_binary.data(),
                                          avatar_binary.size());
        } else {
            avatar_base64 = "";
...
void Handlers::login_user_response(http::response<http::string_body> response, User &user)
{
    ...
    if (!avatar.empty()) {
        ...
        if (!dir.cd("Images")) { user.profile_avatar = ""; }
        boost::uuids::random_generator generator;
        boost::uuids::uuid uuid1 = generator();
        user.profile_avatar = QString::fromStdString(dir.currentPath().toStdString() + to_string(uuid1) + ".png");
    } else {
        user.profile_avatar = "";
    }
}
...
void Handlers::find_friend_response(http::response<http::string_body> response, Friend &new_friend)
{
    ...
    if (!avatar.empty()) {
        ...
        if (!dir.cd("Images")) { new_friend.profile_avatar = ""; }
        boost::uuids::random_generator generator;
        boost::uuids::uuid uuid1 = generator();
        new_friend.profile_avatar = QString::fromStdString(dir.currentPath().toStdString() + to_string(uuid1) + ".png");
    } else {
        new_friend.profile_avatar = "";
    }
}
...
http::request<http::string_body> Handlers::create_chat_request(Chat &chat)
{
    ...
    json::value request_value = {
            {"chat",
             {
                 {"chat_name", chat.name.toStdString()},
...
http::request<http::string_body> Handlers::send_message_request(Message &msg)
{
    ...
    if (!msg.attachment.isEmpty()) {
        ...
        if (file.good()) {
            ...
            beast::detail::base64::encode((void *) image_base64.data(),
                                          image_binary.data(),
                                          image_binary.size());
        } else {
            image_base64 = "";
